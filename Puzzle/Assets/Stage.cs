...
public class Stage : MonoBehaviour
{
    ...
    public void StageInit()
    {
        ...
        if (number == 1)
        {
            if(GetComponentInParent<GroupStage>().minimumStarScore <= PlayerPrefs.GetInt("StarScoreTotal") && PlayerPrefs.GetInt(GetComponentInParent<LevelSelection>().GetLevelName(chapterNumber, number)) < 2)
            {
                stageState = 1;
            }
            else
       ...
       if(PlayerPrefs.GetInt(GetComponentInParent<LevelSelection>().GetLevelName(chapterNumber, number)) == 1) 
        {
            if(number != 1)
                stageState = 1;
        }
        ...
        if (stageState == 0)
        {
            ...
            buttonState[3].gameObject.SetActive(false);            
        }
        ...
        else if (stageState == 3)
        {
            ...
            buttonState[1].gameObject.SetActive(false);
            buttonState[2].gameObject.SetActive(false);
            buttonState[3].gameObject.SetActive(true);
        }
    }
}
